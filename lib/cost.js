	...
	appendRow(costKind) {
		...
		row.remove.addEventListener("click", () => {
			...
			if (costItem.total !== 0) {
				...
				const message = `Eintrag ${title} (${amount}) entfernen?`
...
customElements.define("cost-footer", class extends HTMLElement {
	connectedCallback() {
		...
		this.shadowRoot.innerHTML = `
			<style>
				...
				:host {
					display: grid;
					grid-template-columns: 13ch 25ch 1fr max-content;
		...
		this.elements = {
			total: this.shadowRoot.querySelector("#total span"),
		}
		this.elements.total.innerText = formatAmount(0)
	...
	refresh() {
		...
		this.costSections.forEach((costSection) => {
			total += costSection.total
		})
		this.elements.total.innerText = formatAmount(total)
	}
})
